...
def train():
    ...
    (X_source, y_source), (X_target, _), (X_test, y_test) = \
        create_dataset(n_train=300, n_test=500, noise=0.05, degree=360)
    ...
    with feature_extractor, feature_extractor.enable_register(True):
        for epoch in range(n_epochs):
            for source_data in random_iterator(X_source, y_source, batch_size, infinite=False):
                ...
                optimizer.zero_grad()
                (loss + 100 * regularization_loss).backward()
                optimizer.step()
            ...
            if epoch % 100 == 0:
                ...
                print(
                    'Epoch: {}/{}, Loss: {:.3f}, Reg: {:.3f}, Accuracy: {:.3f}'
                        .format(epoch, n_epochs, loss.item(), regularization_loss.item(), test_accuracy)
                )
            if epoch % 100 == 0:
                plt.clf()
